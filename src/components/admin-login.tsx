...
export function AdminLoginPanel({ configured }: AdminLoginProps) {
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-ivory px-4 text-ink">
      <form className="w-full max-w-md rounded-3xl border border-black/10 bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.05)]" onSubmit={handleSubmit}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-mute">只有管理员可进入知识库管理。</p>
          </div>
        </div>
        ...
        <label className="mt-5 block text-sm">
          <span className="mb-2 block text-mute">管理密码</span>
          <input
            type="password"
            value={password}
        ...
        <button
          ...
          className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-ink px-4 text-sm font-medium text-white transition hover:bg-ink-soft disabled:cursor-not-allowed disabled:bg-[#888]"
        >
