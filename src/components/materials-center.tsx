...
export function MaterialsCenter({ onBack }: MaterialsCenterProps) {
  ...
  const items = useMemo(
    () =>
      DATA.filter((item) => item.cat === cat).filter(
        (item) => !q.trim() || item.title.includes(q.trim()) || (item.copy?.includes(q.trim()) ?? false),
      ),
    [cat, q],
  )
    ...
    <div className="flex flex-col w-full h-full" style={{ background: "#f7f3ec" }}>
      ...
      <div className="@container flex-1 min-h-0 overflow-y-auto lux-scroll px-4 md:px-8 pb-6">
        <div className="shrink-0 pt-4 pb-3 space-y-3">
          <div className="flex items-center gap-2 rounded-full px-4" style={{ background: "#ffffff", border: "1px solid #e6dccb", height: 40 }}>
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 @md:grid-cols-2 @4xl:grid-cols-3 gap-3">
            {items.map((item) => (
              <article key={item.id} className="overflow-hidden rounded-2xl flex flex-col" style={{ background: "#ffffff", border: "1px solid #e6dccb" }}>
                <div className="relative grid place-items-center" style={{ height: 140, background: item.hue }}>
                ...
                <div className="flex-1 flex flex-col p-3.5">
                  ...
                  {item.copy ? (
                    <div className="mt-2.5 rounded-xl p-2.5 whitespace-pre-wrap" style={{ background: "#f7f3ec", color: "#3a322a", fontSize: 12.5, lineHeight: 1.65, maxHeight: 132, overflow: "auto" }}>
                      {item.copy}
                    </div>
                  ) : null}
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    {item.download ? (
                      <button
                        ...
                        style={{ border: "1px solid #e6dccb", color: "#1a1410", fontSize: 12, letterSpacing: "0.06em" }}
                      >
                        <Download size={13} />
                        下载
                      </button>
